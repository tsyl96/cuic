...
#B2B
B2B_Header_file = "B2B Hourly Report For(" + d_out + ").csv"
B2B_total_name = "B2B Total Report" + d_out + time + ".csv"
B2B_out_name= "B2B" + d_out + time + ".csv"
B2Bfinal_filename = "B2B Final" + d_out + ".csv"
B2B_excel_name = "B2B Hourly Report For" + d_out + time + ".xlsx"
...
def open_CCSC():
    ...
    #driver.find_element_by_xpath('//*[@id="reportDrawer_24"]').click()
...
def Header_file_creation():
    with open(str( out_folder / Header_file), mode='w') as file:
        ...
        file_writer.writerow(
            ['Interval', 'CallOffered', 'CallsHandled', 'AbanRate', 'ASA', 'AHT', 'AvgTalkTime', 'AvgHoldTime', 'Short Call' , 'SLPercentage','APS Hourly '])
...
def B2B_Header_file_creation():
    with open(str(out_folder / B2B_Header_file), mode='w') as file:
        file_writer = csv.writer(file, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)
        file_writer.writerow(
            ['Interval', 'CallOffered', 'CallsHandled', 'AbanRate', 'SLPercentage', 'ASA', 'AHT', 'AvgTalkTime',
             'AvgHoldTime'])
    print("\nB2B_Header_file has been created")


if hr == 7:
   B2B_Header_file_creation()
#B2B_Header_file_creation()
...
#### SL details
        ...
        driver.switch_to.default_content()
        driver.implicitly_wait(1)
        driver.find_element_by_xpath('//*[@id="reportDrawer_32"]').click()
        ...
        print("SL Details", SLdetails.text)
        ...
        APS_file = "APS count for(" + d_out + ")" + "-" + str(hr-1) + ".csv"

        # read file
        with open(str(out_folder / APS_file), "r")as file:
            content = file.read()
            print("reading data " + content)
            #reader = csv.reader(file)
            #for row in reader:
              #  print("Reader" + str(row))
               # print(type(row))

        # Write short call data in output file
        APSdata = pd.read_csv(str(out_folder / self.file_output))
        APSdata.insert(10, 'APS Hourly ', content )
        APSdata.to_csv(str(out_folder / self.file_output), index=False)
        print("\nAPS hourly has been added in  " + self.file_output)
...
def B2B_CCSC():
    ...
    driver.find_element_by_id("reportDrawer_9").click()
    driver.implicitly_wait(1)
    driver.find_element_by_id("reportDrawer_34").click()
    driver.implicitly_wait(1)
    driver.find_element_by_id("reportDrawer_35").click()
    print("B2B Success")
B2B_CCSC()

class MIS_B2B_Report:


    def __init__(self, B2B_report):
        self.B2B_report = B2B_report
        print(B2B_report)
        # choose total or hourly
        if B2B_report == "B2B total report":
            self.B2B_start_time = "6"
            self.B2B_file_output = B2B_total_name
            self.B2B_Interval_out = "Total"
            print("Total B2B has been chosen")
        elif B2B_report == "B2B  hourly report":
            self.B2B_start_time = str(hr - 1)
            self.B2B_file_output = B2B_out_name
            self.B2B_Interval_out = insert_hr
            print("Hourly B2B has been chosen")

    # Function for CCSC and short call output automation
    def data_output(self):
        print(self.B2B_start_time)
        print(self.B2B_file_output)
        print(self.B2B_Interval_out)



        try:
            driver.switch_to.frame(driver.find_element_by_id('B2B_CCSC_Dashboard'))
            driver.switch_to.frame(driver.find_element_by_id('filter_iframe'))
        except NoSuchElementException as exception:
            pass

        # Enter start date
        driver.find_element_by_id("8416E19F1000016800DA806C0A54562C-dateParamTextbox").send_keys(d3)

        # Enter end date
        driver.find_element_by_id("8416E19F1000016800DA806B0A54562C-dateParamTextbox").send_keys(d3)

        # Enter start hour,min and second
        driver.find_element_by_id("8416E19F1000016800DA806C0A54562C-timeHourParamTextbox").send_keys(self.B2B_start_time)
        driver.find_element_by_id("8416E19F1000016800DA806C0A54562C-timeMinParamTextbox").send_keys("00")
        driver.find_element_by_id("8416E19F1000016800DA806C0A54562C-timeSecParamTextbox").send_keys("00")

        # Enter end hour,min and second
        driver.find_element_by_id("8416E19F1000016800DA806B0A54562C-timeHourParamTextbox").send_keys(hr - 1)
        driver.find_element_by_id("8416E19F1000016800DA806B0A54562C-timeMinParamTextbox").send_keys("59")
        driver.find_element_by_id("8416E19F1000016800DA806B0A54562C-timeSecParamTextbox").send_keys("59")
        ...
        # get data from table
        web_table = driver.find_element_by_id('dataTable')
        B2B_CallsOffered = web_table.find_element_by_xpath('//*[@id="tbody"]/tr/td[1]')
        B2B_CallsHandled = web_table.find_element_by_xpath('//*[@id="tbody"]/tr/td[2]')
        B2B_ASA = web_table.find_element_by_xpath('//*[@id="tbody"]/tr/td[4]')
        B2B_AHT = web_table.find_element_by_xpath('//*[@id="tbody"]/tr/td[5]')
        B2B_AvgTalkTime = web_table.find_element_by_xpath('//*[@id="tbody"]/tr/td[6]')
        B2B_AvgHoldTime = web_table.find_element_by_xpath('//*[@id="tbody"]/tr/td[7]')
        B2B_AbanRate = web_table.find_element_by_xpath('//*[@id="tbody"]/tr/td[8]')
        B2B_SLPercentage = web_table.find_element_by_xpath('//*[@id="tbody"]/tr/td[10]')

        # Write data in output file
        with open(str(out_folder / self.B2B_file_output), mode='w') as file:
            B2B_file_writer = csv.writer(file, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)
            B2B_file_writer.writerow(
                ['Interval', 'CallOffered', 'CallsHandled', 'AbanRate', 'SLPercentage', 'ASA', 'AHT', 'AvgTalkTime',
                 'AvgHoldTime'])
            B2B_file_writer.writerow(
                [self.B2B_Interval_out, B2B_CallsOffered.text, B2B_CallsHandled.text,
                 B2B_AbanRate.text , B2B_SLPercentage.text + "%", B2B_ASA.text, B2B_AHT.text, B2B_AvgTalkTime.text, B2B_AvgHoldTime.text])

        print("\n B2B Automation done for " + self.B2B_file_output)

B2B_HourlyReport = MIS_B2B_Report("B2B  hourly report")
B2B_HourlyReport.data_output()

# Hourly Output datas add to header file
def B2B_add_header():
    B2B_userDf = pd.read_csv(str(out_folder / B2B_out_name))
    #print(userDf)

    with open(str(out_folder / B2B_Header_file), 'a')as append_total:
        B2B_userDf.to_csv(append_total,index=False ,header=False)
...
B2B_TotalReport = MIS_B2B_Report("B2B total report")
B2B_TotalReport.data_output()

#Combine Hourly and Total Reports
def B2B_output_combine():

    with open(str(out_folder / "B2B Interval.csv"), mode='w') as file:
        B2B_file_writer = csv.writer(file, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)
        B2B_file_writer.writerow(
            ['Interval', 'CallOffered', 'CallsHandled', 'ASA', 'AHT', 'AvgTalkTime', 'AvgHoldTime', 'AbanRate',
            'SLPercentage'])
        i = hr
        while (i < 24):
            B2B_final_interval = str(i) + ":00-" + str(i + 1) + ":00"
            B2B_file_writer.writerow([B2B_final_interval, "", "", " ", "", "", "", "", "" ])
            i += 1

    B2B_total_Df = pd.read_csv(str(out_folder / B2B_total_name))
    #print("ftth total df " total_Df)

    shutil.copyfile(str(out_folder / B2B_Header_file), str(out_folder / B2Bfinal_filename))

    B2B_interval_df = pd.read_csv(str(out_folder / "B2B Interval.csv"), error_bad_lines=False)
    print("interval" + B2B_interval_df)

    with open(str(out_folder / B2Bfinal_filename), 'a')as append_interval:
        B2B_interval_df.to_csv(append_interval, index=False, header=False)

    with open(str(out_folder / B2Bfinal_filename), 'a')as append_final:
        B2B_total_Df.to_csv(append_final, index=False, header=False)

    with ExcelWriter(str(out_folder / B2B_excel_name))as ew:
        pd.read_csv(str(out_folder / B2Bfinal_filename), error_bad_lines=False).to_excel(ew, sheet_name="B2B MIS report", index=False)
    print("\nAutomation done for B2B MIS Report" + ftth_excel_name)
...
def automail():
    ...
    yag.send(
        ...
        attachments=[str(out_folder / excel_name), str(out_folder / ftth_excel_name), str(out_folder / B2B_excel_name)]
    )
